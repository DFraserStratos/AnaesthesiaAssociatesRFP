...
| ASA "seeds the M field" yet AS1-AS4 are also listed as modifier codes in the same table, so the seed could double-count | 1055-1071 | ASA is captured in its own field; the AS group is filtered out of the chip set; the calculator adds the ASA code only when the chip list does not already contain it | `ModifierChips.tsx:24`, `fee.ts:81-84`, test `fee.test.ts:55`. Values logged at `PROGRESS.md` line 89 |
