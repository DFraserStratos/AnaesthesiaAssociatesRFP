...
| ASA "seeds the M field" yet AS1 to AS4 also appear as modifier codes in the same table, so the seed could double-count | 1055-1071 | ASA is captured in its own field, the AS group is filtered out of the picker, and the calculator appends the ASA code only when the selection does not already contain it | `ModifierChips.tsx:28` (band controls since 2026-07-28), `fee.ts:81-84`, test `fee.test.ts:55` | Defensible |
...
| Positioning is "sometimes already built into the base code" and sometimes added as P1 | 1020-1023 | An explicit `absorbsModifierCodes` list per RVG code; the P1 chip is disabled and struck through and the domain's refusal reason renders verbatim | `REQUIREMENTS.md` B3; `rvgCodes.ts:25-40`, `modifierUnits.ts` | Defensible (finding 06.5, the M caption crediting an absorbed chip, was fixed on 2026-07-27 with the band work: the caption now skips every refused code) |
| The modifier table is organised by group with an "Example range" per group (PA 1 to 4, A 1 to 2, AS 0 to 4, OB 0 to 3), and modifiers "depend on a score or category", but the RFP never says only one code per group applies. Stacking, which the build allowed, takes OB to 6, A to 3 and PA to 11 and breaks the RFP's own arithmetic. A demo card was found carrying A1 and A2 together, M = 3 for one patient | 1050-1105 | **Band rule, a picked reading.** PA1 to PA4, A, AS and OB are mutually exclusive bands: the picker swaps a sibling out on tap, and `modifierUnits` refuses a same-band collision with a reason, keeping the first code (first-wins, which preserves the ASA precedence `fee.ts` already had). **PA5 is exempt and stacks** on PA1 to PA4, because it is a distinct later contact (a phone follow-up), not an alternative assessment type. That exception is a deliberate deviation: PA can then reach 5 units, one above the RFP's stated 1 to 4 range, and since the range arithmetic is the whole argument for the rule, it is stated rather than glossed. POSTOP also stacks ("separately itemised, sometimes days later", 1103); ASE, P1 and AI1 are singletons | PROGRESS 2026-07-27 "Modifier bands"; `REQUIREMENTS.md` B3; `modifierCodes.ts` (`EXCLUSIVE_MODIFIER_GROUPS`, `STACKS_WITHIN_GROUP`); on screen as the **Selection** column on the Modifier codes master tab | Needs an answer from AA (labelled on screen, so safe to present) |
...
| The ACCPAY is "the undiscounted payable" while the ACCREC may carry a Type 2 contract discount, and the RFP never says where AA's agency margin is taken | 1400 | The 2026-07-29 user decision supersedes `D-payee-amount`: the prototype shows ACCREC gross, a clearly labelled illustrative 5% AA service fee, and ACCPAY net. Partial receipts authorise the net payable proportionally. The assumption is visible on the pair detail and is not presented as an RFP rule | PROGRESS 2026-07-29 "Xero money flow and illustrative AA fee" | Needs an answer from AA (actual rate, deduction basis and GST treatment) |
...
10. **The outstanding-balance check at intake.** Should it distinguish "open" from "genuinely
    ...
    billing?
11. **GST and the agency margin.** We invoice GST-exclusive lines plus 15% and illustrate a 5% AA
    service fee deducted before ACCPAY. Are the invoice GST treatment, fee rate, deduction basis and
    fee GST treatment correct, given the RFP calls the ACCPAY "the undiscounted payable"?
...
21. **Modifier groups.** Your per-group example ranges only hold if exactly one code per group
    applies, so we made PA1 to PA4, A, AS and OB pick-one and swap on tap. Is that right? And the
    follow-on: is a pre-assessment follow-up separately claimable **on top of** the assessment
    itself? We allow PA5 to stack, which takes PA to 5 units, one above your stated 1 to 4 range.
