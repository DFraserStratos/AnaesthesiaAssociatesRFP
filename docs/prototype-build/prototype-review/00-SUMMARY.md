...
| P5 | Mock backend, versioned `localStorage`, visible reset, no network | BUILT **(self-checked)** | `src/store/appStore.ts:112` `PERSIST_VERSION = 7` and `:182` persist; "Reset demo data" at `src/apps/demo/DemoControlPanel.tsx:281`; repo-wide grep for `fetch(` in `src` returns nothing outside tests |
