...
1. **Pre-payment flow** (B7): the pre-payment flag is typed per the RFP — **full** or **split** —
   ...
   the agreed deposit portion for split. **Collected-before-procedure is a real gate, not an
   ...
   complete while its pre-invoice is unpaid (via the Phase 10 payment simulation; a seeded paid
   state covers the demo where payment has genuinely landed). Where the office needs to proceed
...
3. **Billing monitor** (admin nav, replacing the placeholder; A5): pipeline table per authorised
   ...
   - **Failure demo:** seed or trigger one failure (e.g. a card whose governing contract was effective-dated out between sanity check and run) showing an error state with a readable message, plus **"resolve & retry"** (fix the data → re-run that card).
...
- [ ] Pre-payment card (split), unpaid: attempting to mark the Outcome complete is genuinely blocked (not just warned); the office's override action (with a reason) lifts it, and the override shows as a flagged state on the card everywhere it appears. Paying the deposit (Phase 10 simulation) also clears the block without an override. After capture + authorise, the balance invoice covers exactly the remainder (deposit + balance = full fee). A full-pre-payment case behaves per the recorded decision.
