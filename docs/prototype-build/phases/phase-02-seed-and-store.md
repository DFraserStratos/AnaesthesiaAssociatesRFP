...
- **Lifecycle guards** (`src/store/lifecycle.ts`):
  ...
  - `submitList` — blocked unless **every Card is marked Completed** (the RFP rule: "a List cannot be marked SUBMITTED unless all its Cards are correctly completed"; validation gates completion, completion gates submission — validation alone is not enough).
  - `authoriseList` — locks Cards immutable; emits a `listAuthorised` event Phase 08 will consume.
...
Reset (with confirm) and clock display + advance-day controls go live in `/demo/control`
(replacing Phase 00's disabled placeholders for those two).
...
card rejected even when all cards would pass validation; anaesthetist-edit-of-SUBMITTED rejected;
office-edit-of-SUBMITTED allowed; any-edit-of-AUTHORISED rejected; List reassignment preserves card
...
List; setAvailability restatuses a Free list but flags (not mutates) a booked one;
...
same statuses/surgeons a fresh seed would give them); reset
...
- [ ] Data inspector: 2 Lists per anaesthetist per day across the horizon; every status colour in use; the seeded SUBMITTED lists, split-billing card, pre-payment patient, and the all-day booking (both Lists same hospital/surgeon) all findable.
