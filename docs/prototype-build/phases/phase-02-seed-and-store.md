...
- **Clock:** `advanceDays(n)` store action; expose a subscription point future phases hook (reconciliation poll, archive job).
...
List; setAvailability restatuses a Free list but flags (not mutates) a booked one; reset
...
- [ ] Advance-day moves `today()` and the inspector reflects it.
