...
6. **Availability screens** (per `Mobile Availability.dc.html`): other-anaesthetists day view —
   ...
   a List. The reconciliation pass reflects it instantly: Free/empty sessions restatus on the
   canvas; a session that already holds Cards is flagged as a conflict for the office instead of
   being silently changed (RFP: availability is an independent master calendar *reconciled
   against* the canvas, not merged into List records).
